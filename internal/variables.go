...
//
// If the version was not set via ldflags at build time, returns
// "(undefined)". Any leading "v" or "V" prefix is stripped and the
...
//
// The stage is set via ldflags and typically matches the branch that
// produced the build (e.g. "main", "staging"). Returns "(undefined)"
// when not set.
func Stage() string {
...
//
// Set via ldflags at build time. Returns "(undefined)" when the binary
// was not built through the standard pipeline.
...
//
// A build is local when version, stage, or gitCommit is empty, which
// means it was not produced by the CI pipeline. This affects the output
// of VersionString.
func IsLocal() bool {
